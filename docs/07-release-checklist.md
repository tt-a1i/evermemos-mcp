# Release Checklist (Competition v0.5.3)

## 1) Goal
Ship a stable competition release (`v0.5.3`) and ensure all submission assets are complete and verifiable.
...
```bash
git add .
git commit -m "release: prepare v0.5.3 competition package"
git tag v0.5.3
...
## 5) GitHub Release
- [x] Create GitHub release for `v0.5.3`
...
```bash
gh release create v0.5.3 --title "v0.5.3" --notes-file CHANGELOG.md
```
