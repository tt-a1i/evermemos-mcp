...
台词（三段式）：
1. “痛点：AI 会话切换后容易丢上下文。”  
2. “方案：我们用 `space_id` 做隔离，用 `recall/briefing/forget` 构成可控记忆闭环。”  
...
2. "Solution: we isolate memory by `space_id`, and use `recall/briefing/forget` as a controllable memory loop."  
